...
geo %>% st_as_sf(coords = c("longitude", "latitude"), crs = 4326) -> geo_sf

koppen_raster <- rast('./data/reference/Beck_KG_V1_future_0p5.tif')

geo$koppen <- terra::extract(koppen_raster, vect(geo_sf))[,2]

data_clean %>% left_join(geo, by = "location_clean", relationship = "many-to-many") -> data_temp

data_temp$koppen %>% case_match(
  c(1:3) ~ "tropical",
  c(4:7) ~ "arid",
  c(8:16) ~ "temperate",
  c(17:28) ~ "cold",
  c(29:30) ~ "polar") -> data_temp$koppen_char

# get season from month and hemisphere

data_temp$hemisphere <- ifelse(data_temp$latitude >= 0, "north", "south")
...
month(data_temp$date) -> data_temp$month

data_temp$season <- mapply(get_season, data_temp$month, data_temp$hemisphere)

data_temp$minutes_from_midnight <- (hour(data_temp$time) * 60 + minute(data_temp$time))*2*pi/1440

data_temp$summary %>% tolower() -> summaries
str_detect(summaries, "thunder|lightning") -> data_temp$thunder_lightning
